const SCORE_CARD_PATH = "symbols";

/**
 * Base URL of the RichMonks API. Configured per environment via
 * NEXT_PUBLIC_API_BASE_URL; never hardcoded here.
 * @returns {string|null}
 */
...
export function normalizeScoreCardItem(row) {
  ...
  const changePct = Number(row.price_change);
 ...
 * Expects `result` to be the symbol list.
 * @param {unknown} data
...
export async function fetchMarketTickerItems() {
  ...
  if (!url) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        "[market] NEXT_PUBLIC_API_BASE_URL is not set — ticker will stay empty.",
      );
    }
    return [];
  }
